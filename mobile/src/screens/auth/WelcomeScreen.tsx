    ...
    <View style={styles.container}>
      <View style={styles.hero}>
        <View style={styles.badge}>
          <PawPrintIcon size={30} color={color.accent} weight="fill" />
        </View>
      ...
      <View style={styles.providers}>
        <ProviderButton
          ...
          icon={<GoogleLogoIcon size={18} color={color.text} weight="bold" />}
          busy={busy === 'google'}
          disabled={busy !== null}
        ...
        <ProviderButton
          ...
          icon={<FacebookLogoIcon size={18} color={color.text} weight="fill" />}
          busy={busy === 'facebook'}
          disabled={busy !== null}
        ...
        <ProviderButton
          ...
          icon={<AppleLogoIcon size={18} color={color.text} weight="fill" />}
          busy={busy === 'apple'}
          disabled={busy !== null}
      ...
      <Button
        ...
        variant="secondary"
        block
        icon={<EnvelopeSimpleIcon size={17} color={color.text} />}
        disabled={busy !== null}
...
function ProviderButton({
  ...
  return (
    <Button
      ...
      variant="secondary"
      block
      icon={busy ? <ActivityIndicator size="small" color={color.text} /> : icon}
      disabled={disabled}
      onPress={onPress}
    />
  );
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: color.bg, padding: 24, paddingTop: 72, gap: 20 },
  ...
  title: { fontSize: 30 },
  tagline: { fontSize: 14.5, color: 'rgba(233,233,237,0.6)', textAlign: 'center' },
  providers: { gap: 10 },
  dividerRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  dividerLine: { flex: 1, height: StyleSheet.hairlineWidth, backgroundColor: color.divider },
  dividerLabel: { fontSize: 12, color: 'rgba(233,233,237,0.5)' },
  error: { fontSize: 12.5, color: color.amber, textAlign: 'center' },
  footer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 'auto', paddingTop: 12 },
  footerText: { fontSize: 13, color: 'rgba(233,233,237,0.6)', fontFamily: font.body },
});
